        ...
        <div class="inputs-list profile-container" id="profile">
            ...
            <div class="">
                @if(config('sanctum.allow_external_communication'))
                    @if(config('sanctum.allow_user_token'))
                        <button class="btn-lg-fill" onclick="toggleAccessTokensPanel(true)">{{ $translation["AccessTokens"] }}</button>
                    @endif
                    @include('partials.home.components.api-info')
                @endif

                <button class="btn-lg-fill delete-btn top-gap-2" onclick="clearPersonalData()">{{ $translation["ClearLocalData"] }}</button>
            </div>
